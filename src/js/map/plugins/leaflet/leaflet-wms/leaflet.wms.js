...
wms.Source = L.Layer.extend({
    ...
    'getFeatureInfo': function(point, latlng, layers, callback) {
        ...
        function done(result) {
            self.hideWaiting();
            var text = self.parseFeatureInfo(result, url);
            callback.call(self, latlng, text);
        }
    },
    ...
    'ajax': function(url, callback) {
        ...
        xhr.withCredentials = true; // Indica que se deben incluir las credenciales en la solicitud
    ...
    'showFeatureInfo': function(latlng, info) {
        ...
        if (infoHtml > 0) { // check if info has any content, if so shows popup
