...
function parseFeatureInfoJSON(info, idTxt, title) {
  ...
  if (info.features.length > 0) {
    ...
    infoAux += '<div style="padding:1em; border: none" class="individualFeature">';
    infoAux +=
      '<h4 style="margin:1em 0; font-weight: 800; border: none" >' +
      title +
      "</h4>";
    infoAux += "<ul>";
...
function adaptToImage(imgDiv) {
  ...
  if (img.naturalHeight > 24) {
    ...
    img.src.includes("svg") ||
      ...
      ? max_url_img
      : (max_url_img +=
        ";fontAntiAliasing:true;wrap:true;wrap_limit:200;fontName:Droid Serif;");
    ...
    resize_img_icon.onclick = (event) => {
      if (container_expand_legend_grafic.getAttribute("load") === "true") {
        ...
        container_expand_legend_grafic.style.backgroundColor = "transparent";
        resize_img_icon.innerHTML =
