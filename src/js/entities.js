...
class ImpresorGrupoHTML extends Impresor {
  imprimir(itemComposite) {
    ...
    return (
      ...
      (itemComposite.descripcion ? itemComposite.descripcion : "") +
      "' href='#" +
      itemComposite.seccion +
      "'>" +
...
class LayersInfoWMS extends LayersInfo {
  ...
  _parseRequest(_gestorMenu) {
    ...
    if (!document.getElementById("temp-menu")) {
      $("body").append('<div id="temp-menu" class="temp" style="display:none"></div>');
    }
    ...
    $.ajax({
      ...
      success: function (data) {
        var xmlDoc = data
        var capability = xmlDoc.querySelector("Capability");
        var keywordHtml = xmlDoc.querySelector("Keyword");
        var keyword = keywordHtml ? keywordHtml.innerText : "";
        var abstractHtml = xmlDoc.querySelector("Abstract");
        var abstract = abstractHtml ? abstractHtml.innerText : "";
        var layer = capability.querySelector("Layer")
        var capas_layer = layer.querySelectorAll("Layer");

        var items = [];

        capas_layer.forEach((layer, index) => {
          var iName = layer.querySelector("Name").innerHTML;
          if (thisObj.isAllowebLayer(iName)) {
            var iTitle = layer.querySelector("Title").innerHTML;
            iTitle = thisObj.formatLayerTitle(iName, iTitle);
            var iAbstract = layer.querySelector("Abstract").innerHTML;
            iAbstract = thisObj.formatLayerAbstract(iName, iAbstract);
            var keywordsHTMLList = layer.querySelectorAll("keywordlist keyword");
            var keywords = Array.from(keywordsHTMLList).map(keyword => keyword.innerText);
            var iBoundingBox = layer.querySelector("BoundingBox");
            var iSrs = null;
            var iMaxY = null;
            var iMinY = null;
            var iMinX = null;
            ...
            if (iBoundingBox) {
              if (iBoundingBox.attributes.hasOwnProperty('srs')) {
                iSrs = iBoundingBox.attributes.srs.nodeValue;
              } else if (iBoundingBox.attributes.hasOwnProperty('crs')) {
                iSrs = iBoundingBox.attributes.crs.nodeValue;
              }

              iMaxY = iBoundingBox.attributes.maxy ? iBoundingBox.attributes.maxy.nodeValue : null;
              iMinY = iBoundingBox.attributes.miny ? iBoundingBox.attributes.miny.nodeValue : null;
              iMinX = iBoundingBox.attributes.minx ? iBoundingBox.attributes.minx.nodeValue : null;
              iMaxX = iBoundingBox.attributes.maxx ? iBoundingBox.attributes.maxx.nodeValue : null;
            }
            var ilegendURLaux = layer.querySelector("Style").innerHTML;
            ...
            if (thisObj.icons) {
              ...
              ilegendURL = divi.querySelector("onlineresource") ? divi.querySelector("onlineresource").getAttribute("xlink:href") : null;
            }

            var capa;
            if (thisObj.type == "wmslayer_mapserver") {
              capa = new CapaMapserver(
                iName,
                iTitle,
                iSrs,
                thisObj.host,
        ...
        success: function (data) {
            ...
            capas_layer.forEach((layer, index) => {
                ...
                if (thisObj.isAllowebLayer(iName)) {
                    ...
                    let divi = document.createElement("div");
                    divi.innerHTML = ilegendURLaux;
                    var ilegendURL = divi.querySelector("onlineresource") ? divi.querySelector("onlineresource").getAttribute("xlink:href") : null;
...
class GestorMenu {
  ...
  cleanAllLayers() {
    ...
    this._getLazyInitLayersInfoCounter = 0
